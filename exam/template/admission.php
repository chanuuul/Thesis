      ...
      <div class="container">
        <div class="az-content-body">
          <div class="az-dashboard-one-title">
            <div>
              <h2 class="az-dashboard-title">Admission List</h2>
...
<input type="search" class="light-table-filter email form-control" data-table="order-table" placeholder="Search"><br>
              ...
              <tbody>
              		<?php
include('../../connect.php');
$result = mysql_query("SELECT * FROM admission WHERE status = 'Approved' ORDER BY ID DESC");
while($row = mysql_fetch_array($result))
{
	$id11 = $row["id"];
	$status = $row["status"];
...
$ab = str_pad($id11, 4, '0', STR_PAD_LEFT); 
						...
						$resultc = mysql_query("SELECT * FROM login WHERE uname = '$username' ORDER BY ID DESC");
						...
						if($countc == 0) {
						echo '<a href="add.php?username='.$row['username'].'&email='.$row['email'].'" class="btn btn-success" style="width:100%"  rel="facebox">Create Examination Account</a> ';
						}
						
						echo'  <a href="#" class="btn btn-primary" style="width:100%;margin-top:10px" onclick="window.location=\'view1.php?id='.$row['id'].'\'">View Information</a> ';
						$resulta = mysql_query("SELECT * FROM grade WHERE student = '$username'");

						//echo '<a style="width:100%;margin-top:5px" class="btn btn-danger" href="delete2.php?id='.$row["id"].'" onclick="return confirm(\'Are you sure you want to delete this record?\')">Decline Application</a></td>';
                        echo'</tr>';
}
?>                     
