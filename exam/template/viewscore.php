...
<table class="table table-striped table-bordered table-hover" id="data2" style="display:none">
                                    <thead>
                                        <tr>
                                            <th>Student Name</th>
                                            <th>Score</th>
					...
					$result = mysql_query("SELECT `score`,number,student, quiz, COUNT(*) AS `count` FROM results1 WHERE quiz = '$quiz' AND score = 'correct' GROUP BY quiz");
					while($row = mysql_fetch_array($result))
					{
						$student = $row['student'];
						// student information								
							...
							$resultaa = mysql_query("SELECT * FROM admission WHERE username = '$uu'");
							while($rowaa = mysql_fetch_array($resultaa))
							{
								$name =$rowaa['firstname'].' '.$rowaa['middlename'].' '.$rowaa['lastname'];
