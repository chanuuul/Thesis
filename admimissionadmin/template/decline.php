...
//end sms
	...
	$type = 'Exam';
mysql_query("UPDATE admission SET status='Declined'  WHERE username =  '$username'");
mysql_query("UPDATE filling SET status='Declined'  WHERE username =  '$username'");
$type = 'Admission';
...
$notification ='Congratulations. You are not qualified to take the Sta Teresa College Admission. ';
$message ='Congratulations. You are not qualified to take the Sta Teresa College Admission Test. ';

$date = date('Y-m-d');
$save2=mysql_query("INSERT INTO notification (notification,date,student,type) VALUES ('$notification','$date','$username','$type')");
mail($to, $subject, $message, $headers);	

 echo "<script type=\"text/javascript\">alert('Information has been updated');window.location.href = 'admission.php';</script>"; 
